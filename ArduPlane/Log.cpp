...
struct PACKED log_Throwwater {
    LOG_PACKET_HEADER;
    uint64_t time_us;
    int16_t throw_or_not;
    float dis_chui;
    float dis_yan;
    float distancecur;
    float alt_caped;
...
void Plane::Log_Write_Throwwater()
{
    struct log_Throwwater pkt = {
        LOG_PACKET_HEADER_INIT(LOG_THROWW_MSG),
        time_us          : AP_HAL::micros64(),
        throw_or_not     : (int16_t)plane.ThroworNot,
        dis_chui         : plane.tdistance_chui,
        dis_yan          : plane.tdistance_yan,
        distancecur      : plane.tdistance_cur,
        alt_caped        : plane.captured_altitude, 
        alt_now          : plane.current_altitude
    };
...
// @Field: AltL: target alt frame (Location)
    ...
    { LOG_THROWW_MSG, sizeof(log_Throwwater), 
      "THRW", "Qhfffff", "TimeUS,Throw,DisChui,DisYan,DisCur,AltCap,AltNow", "s-mmmmm", "F------", true }, 
};
