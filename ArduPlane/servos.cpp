...
void Plane::servos_output(void)
{
    ...
    if (control_mode == &mode_auto) {
    const uint8_t rudder_ch_idx = 3; // 物理通道4（索引3）
    SRV_Channel* rudder_ch = SRV_Channels::srv_channel(rudder_ch_idx);
    
    // 1. 获取舵机中立值（单位：μs）
    const int16_t neutral_pwm = rudder_ch->get_trim(); 
    
    // 2. 获取当前PWM输出值
    int16_t current_pwm = rudder_ch->get_output_pwm();
    
    // 3. 计算真实偏转量（相对中立位置）
    int16_t deflection = current_pwm - neutral_pwm;
    
    // 4. 舵量翻倍（基于偏转量）
    deflection *= 2;
    
    // 5. 约束到舵机行程范围（防止超限）
    int16_t new_pwm = neutral_pwm + deflection;
    new_pwm = constrain_int16(new_pwm, 
                              rudder_ch->get_output_min(),  // SERVO4_MIN
                              rudder_ch->get_output_max()); // SERVO4_MAX
    
    // 6. 更新输出
    rudder_ch->set_output_pwm(new_pwm);
}
