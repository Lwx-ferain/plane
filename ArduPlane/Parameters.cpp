...
const AP_Param::GroupInfo ParametersG2::var_info[] = {
    ...
    // @Param: THROWW
    // @DisplayName: 
    // @Description: The type of throw-water
    // @Values: 0: none, 1: auto, 2: test
    // @User: Advanced
    AP_GROUPINFO("TW_TEST", 39, ParametersG2, throwwater, 0),  //一般设为0；实验时：1输出控的投放PWM，2输出PWM=1800
    AP_GROUPINFO("TW_DELAY", 40, ParametersG2, throwwater_delay, 0.0f),  //投放的机械延迟，测试可得，一般取0.2-0.3
    AP_GROUPINFO("TW_LONG", 41, ParametersG2, throwwater_target_lng, 0),  //靶标经度
    AP_GROUPINFO("TW_LAT", 42, ParametersG2, throwwater_target_lat, 0),  //靶标纬度
    AP_GROUPINFO("AIRL_FF_DIS", 43, ParametersG2, Airline_Feedforwad_distance, 0),  //自动航线规划中的航线前视距离，航线较长时可取5m，10m，也可取0
    AP_GROUPINFO("AIRB_LMAX", 44, ParametersG2, max_pwm_left, 2000),  //左侧副翼最大PWM
    AP_GROUPINFO("TW_R_PICKLE", 45, ParametersG2, Throwwater_Radius_Pickle, 0),  //允许投放的范围半径，三公斤一区5m，二区10m
    AP_GROUPINFO("TW_CTRL_MIX", 46, ParametersG2, Throwwater_mix , 0.5),  //设置自动模式下混控参数
    AP_GROUPINFO("TW_PWM_CCRP_ACT", 47, ParametersG2, Airbrake_situation, 1),  //是否允许空气刹车？？

    AP_GROUPINFO("AIRB_RECORD",48, ParametersG2, Airbrake_record, 0), //记录副翼endpoint，设置为1记录一次，自动复位为0
    AP_GROUPINFO("AIRB_LMIN",49, ParametersG2, min_pwm_left, 800), //
    AP_GROUPINFO("AIRB_RMIN", 50, ParametersG2, min_pwm_right, 800),  //
    AP_GROUPINFO("TW_YAN_PICKLE", 51, ParametersG2, Throwwater_Yan_Pickle, 0),  //允许投放的沿飞行方向到靶心的距离，较大时取5m
    AP_GROUPINFO("TW_REC_POS", 52,ParametersG2,Throwwater_record_position, 0),  //记录飞机位置为靶标位置，每次记录后自动复位
    AP_GROUPINFO("TW_UP_ALT", 53,ParametersG2,Throwwater_extra_altitude, 0),//  投放高度调整值，根据当前高度增加或减少，单位米
    AP_GROUPINFO("AIRB_RMAX", 54, ParametersG2, max_pwm_right, 2000),  //是否开启手动投放，0为飞控全权接管投放，1为手动信号覆盖飞控控制
    AP_GROUPINFO("AIRL_MAN_NEU", 55, ParametersG2,man_neutral , 1500),  // 手动模式平尾中立
    AP_GROUPINFO("TW_TW_PWM", 56, ParametersG2, Throwater_throw_pwm, 1100),  //投放时pwm
    AP_GROUPINFO("TW_NOR_PWM", 57, ParametersG2, Throwater_normal_pwm, 1800), //不投放时pwm

    AP_GROUPINFO("AIRB_L_PWM", 58, ParametersG2, Airbrake_left_pwm, 1800),  //左侧空速刹车舵机PWM
    AP_GROUPINFO("AIRB_R_PWM", 59, ParametersG2, Airbrake_right_pwm, 1800), //右侧空速刹车舵机PWM
    AP_GROUPINFO("AIRB_CH", 60, ParametersG2, Airbrake_rc_channel, 14), //空速刹车通道
    AP_GROUPINFO("AIRB_L_PRE",61, ParametersG2, Airbrake_pre_left, 1500), //空速刹车记录舵机中立位置
    AP_GROUPINFO("AIRB_R_PRE",62, ParametersG2, Airbrake_pre_right, 1500), //空速刹车记录舵机中立位置
    AP_GROUPINFO("AIRL_AUTO_NEU",63, ParametersG2, auto_neutral, 1500), //自动模式平尾中立
